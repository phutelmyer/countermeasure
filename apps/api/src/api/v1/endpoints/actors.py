...
async def create_actor(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create actor: {e!s}")
...
async def get_actor(
    ...
    try:
        actor = await ActorService.get_actor(
            db=db, actor_id=actor_id, tenant_id=current_user.tenant_id
        )
        return ActorResponse.model_validate(actor)
...
async def list_actors(
    ...
    include_related: bool = Query(
        False, description="Include related campaigns and malware"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> ActorListResponse:
