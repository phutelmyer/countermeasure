...
class SigmaParser:
    ...
    async def parse_rule_file(self, file_path: Path) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            logger.warning(f"YAML parsing error in {file_path}: {e!s}")
            return None
        except Exception as e:
            logger.error(f"Error parsing {file_path}: {e!s}")
            return None
    ...
    def _map_performance_impact(self, rule_data: dict[str, Any]) -> str:
        ...
        if wildcard_count > 5 or regex_patterns > 10:
            return "high"
        if wildcard_count > 2 or regex_patterns > 5:
            return "medium"
        return "low"
    ...
    async def build_detection_create(
        self, rule_data: dict[str, Any]
    ) -> DetectionCreate | None:
        """
        ...
        try:
            ...
            if not title:
                logger.warning(
                    f"Rule missing title: {rule_data.get('_file_name', 'unknown')}"
                )
                return None
            ...
            if not severity_id:
                logger.warning(
                    f"Unknown severity mapping: {sigma_level} -> {our_severity}"
                )
            ...
            if file_path:
                ...
                try:
                    ...
                    source_url = (
                        f"https://github.com/SigmaHQ/sigma/blob/master/{relative_path}"
                    )
                except ValueError:
            ...
            detection_create = DetectionCreate(
                ...
                status=self.STATUS_MAPPING.get(
                    rule_data.get("status", "testing").lower(), "testing"
                ),
            ...
            detection_create.__dict__["_sigma_metadata"] = {
                ...
                "platform_tags": [
                    tag for tag in tags if tag in ["windows", "linux", "macos"]
                ],
                "references": rule_data.get("references", []),
            ...
            logger.error(f"Error building detection from rule: {e!s}")
            return None
    ...
    async def parse_rules(self, rule_files: list[Path]) -> list[DetectionCreate]:
        ...
        for file_path in rule_files:
            try:
                ...
                logger.error(f"Unexpected error parsing {file_path}: {e!s}")
                failed_count += 1

        logger.info(
            f"Parsed {len(detections)} rules successfully, {failed_count} failed"
        )
        return detections
