        ...
        try:
            ...
            for tag_name in tag_names:
                tag_id = await self.api_client.get_or_create_tag(
                    name=tag_name, description=f"Auto-generated tag for {tag_name}"
                )
                if tag_id:
            ...
            logger.error(f"Error enriching detection '{detection.name}': {e!s}")
            return detection

    async def enrich_detections(
        self, detections: list[DetectionCreate]
