...
from src.core.config import load_mitre_config
...
class MitreCollector(AbstractCollector):
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Authentication error: {e!s}")
            return False
    ...
    async def fetch(self) -> dict[str, Any]:
        ...
        try:
            self.logger.info(
                f"Fetching MITRE ATT&CK data from {self.config.mitre_stix_url}"
            )
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(self.config.mitre_stix_url) as response:
                    if response.status == 200:
                        ...
                        self.logger.info(
                            f"Successfully fetched STIX data with {len(data.get('objects', []))} objects"
                        )
                        return data
                    raise Exception(f"Failed to fetch data: HTTP {response.status}")

        except Exception as e:
            self.logger.error(f"Failed to fetch MITRE data: {e!s}")
            raise
    ...
    async def parse(self, raw_data: dict[str, Any]) -> MitreData:
        ...
        try:
            ...
            self.logger.info(
                f"Parsed {len(tactics)} tactics, {len(techniques)} techniques, {len(groups)} groups"
            )

            return MitreData(tactics=tactics, techniques=techniques, groups=groups)

        except Exception as e:
            self.logger.error(f"Failed to parse MITRE data: {e!s}")
            raise
    ...
    def _parse_tactic(self, stix_obj: dict[str, Any]) -> dict[str, Any]:
        ...
        stix_uuid = (
            stix_id.replace("x-mitre-tactic--", "")
            if stix_id.startswith("x-mitre-tactic--")
            else ""
        )
    ...
    async def submit(self, enriched_data: MitreData) -> CollectionResult:
        ...
        try:
            ...
            self.logger.info("Submitting MITRE data via unified import endpoint...")

            # Submit all data via unified endpoint
            response = await self.api_client.post(
                "/api/v1/mitre/import", json=import_data
            )
            ...
            if response:
                ...
                if "tactics" in details:
                    ...
                    self.logger.info(
                        f"Tactics: {tactics_stats.get('created', 0)} created, {tactics_stats.get('updated', 0)} updated"
                    )

                if "techniques" in details:
                    techniques_stats = details["techniques"]
                    self.logger.info(
                        f"Techniques: {techniques_stats.get('created', 0)} created, {techniques_stats.get('updated', 0)} updated"
                    )

                if "groups" in details:
                    groups_stats = details["groups"]
                    self.logger.info(
                        f"Groups: {groups_stats.get('created', 0)} created, {groups_stats.get('updated', 0)} updated"
                    )

                self.logger.info(
                    f"Import completed: {total_created} created, {total_updated} updated"
                )
            ...
            self.logger.error(f"Submission failed: {e!s}")
            return CollectionResult(
                total_processed=total_processed,
                successful=successful,
                failed=total_processed,
...
async def main():
    ...
    parser.add_argument(
        "--api-url", default="http://localhost:8000", help="Countermeasure API URL"
    )
    ...
    parser.add_argument(
        "--batch-size", type=int, default=50, help="Batch size for API submissions"
    )
    parser.add_argument(
        "--disable-tactics", action="store_true", help="Disable tactics collection"
    )
    parser.add_argument(
        "--disable-techniques",
        action="store_true",
        help="Disable techniques collection",
    )
    parser.add_argument(
        "--disable-groups", action="store_true", help="Disable groups collection"
    )

    args = parser.parse_args()

    # Get password from args or environment
    password = args.password or os.getenv(
        "COUNTERMEASURE_PASSWORD", "CountermeasureAdmin123!"
    )
